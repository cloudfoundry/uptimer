...
var _ = Describe("CfCmdGenerator", func() {
	...
	Describe("Push", func() {
		It("Generates the correct command", func() {
			expectedCmd := exec.Command("cf", "push", "appName", "-p", "path/to/app", "-b", "binary_buildpack", "-c", "./start-command", "-i", "3", "-m", "64M", "-k", "16M")
			...
			cmd := generator.Push("appName", "path/to/app", "./start-command", 3)
