...
var _ = Describe("CfCmdGenerator", func() {
	...
	Describe("Push", func() {
		It("Generates the correct command", func() {
			expectedCmd := exec.Command("cf", "push", "appName", "-d", "apps.example.com", "-p", "path/to/app", "-b", "binary_buildpack", "-c", "./start-command", "-i", "3", "-m", "1024M")
			...
			cmd := generator.Push("appName", "apps.example.com", "path/to/app", "./start-command", 3)
